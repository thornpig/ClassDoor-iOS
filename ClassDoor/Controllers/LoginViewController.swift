...
//                    BackendDataService.shared.patchWithID($0!.id!, type: type(of: $0!), data: ["last_name": "\(self.user!.lastname)wahaha"]) {print($0!.lastname)}
                    let d0 = Dependent(firstname: "Adela", lastname: "Zhu", dependencyID: $0!.modelObj.id!)
                    BackendDataService.shared.save(DependentBackendResource(of: d0)) {
                        dp in
                        BackendDataService.shared.getWithID(dp!.modelObj.dependencyID, type: UserBackendResource.self) {
                            userResource in
                            print(userResource!.modelObj.dependents![0].id!)
                        }
                    }
                }
            }
